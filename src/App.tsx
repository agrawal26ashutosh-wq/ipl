...
${p2sq.map(p=>{const mq=p2own.marquee.includes(p.name);const pts=(scores[p.name]?.matchPts||[]).slice(PHASE2_FROM_MATCH-1).reduce((a,b)=>a+b,0);return `<div class="mh-row" onclick="window.__opm('${esc(p.name)}')" style="cursor:pointer${mq?';background:rgba(124,77,255,0.07)':''}"><span>${p.name}${mq?mqBadge(true):''}</span><span class="mh-pts" style="color:#7C4DFF">${pts}</span></div>`;}).join('')}`;
  ...
  function renderRawStats(){
    ...
    const stot=(c:string|number,s='')=>`<td style="padding:5px 8px;border:1px solid #1e2a3a;font-size:11px;font-family:'JetBrains Mono',monospace;white-space:nowrap;position:sticky;z-index:1;background:#0a0f18;${s}">${c}</td>`;
    const totRow=`<tr style="background:#0a0f18;font-weight:700;">${stot('TEAM TOTAL','left:0;min-width:140px;color:#FFD700;font-weight:700;font-size:12px;')}${stot('','left:140px;min-width:48px;')}${stot('','left:188px;min-width:48px;')}${stot(tf,`left:236px;min-width:64px;color:#FFD700;text-align:right;font-weight:700;font-size:13px;border-right:2px solid #2A3550;`)}${totMC}</tr>`;
    ...
    const stickyTh=(c:string,s='')=>`<th style="padding:5px 8px;border:1px solid #1e2a3a;font-size:10px;font-weight:700;letter-spacing:1px;background:#0a0f18;white-space:nowrap;text-align:center;position:sticky;left:0;z-index:2;${s}">${c}</th>`;
    const hdrs=`${stickyTh('Player','text-align:left;min-width:140px;left:0;')}${stickyTh('Role','min-width:48px;left:140px;')}${stickyTh('IPL','min-width:48px;left:188px;')}${stickyTh('Points','color:#FFD700;min-width:64px;left:236px;border-right:2px solid #FFD700;')}${mHdrs}`;
    return `<details style="margin-bottom:10px;"><summary style="cursor:pointer;font-size:10px;font-weight:700;letter-spacing:2px;color:#7A8BAA;text-transform:uppercase;padding:6px 0;user-select:none">▶ MATCH INDEX (click to expand)</summary><div style="background:#0a0f18;border:1px solid #2A3550;border-radius:6px;padding:8px 12px;margin-top:6px;max-height:200px;overflow-y:auto;">${legend}</div></details><div style="overflow-x:auto;-webkit-overflow-scrolling:touch;"><table style="border-collapse:collapse;width:100%;"><thead><tr>${hdrs}</tr></thead><tbody>${rows}${totRow}</tbody></table></div>`;
  }
