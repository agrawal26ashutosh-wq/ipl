...
function extractMatchNumber(name:string):number {
  ...
  const m2=name.match(/match\s+(\d+)/i);
...
function fuzzyMatch(apiName:string|null|undefined):string|null{
  ...
  if(apiParts.length>=2){
    const apiFirst=apiParts[0],apiLast=apiParts[apiParts.length-1];
    f=eligible.find(p=>{
...
export default function App(){
  ...
  function doSelectTrack(i:number){setCurrentTrack(i);if(ytReadyRef.current&&ytPlayerRef.current){ytPlayerRef.current.loadVideoById(TRACKS[i].id);ytPlayerRef.current.setVolume(volume);setIsPlaying(true);}}
  function togglePlay(){if(!ytReadyRef.current){pendingPlayRef.current=true;showToast('Player loading… tap play again in a moment');return;}if(isPlaying) ytPlayerRef.current?.pauseVideo(); else ytPlayerRef.current?.playVideo();}
  ...
  useEffect(()=>{
    function handleFirstClick(){if(autoPlayDone.current) return;autoPlayDone.current=true;document.removeEventListener('click',handleFirstClick);if(ytReadyRef.current&&ytPlayerRef.current&&!isPlaying) ytPlayerRef.current.playVideo(); else pendingPlayRef.current=true;}
  ...
  function renderLeaderboard(){
    const ranked=sortedTeams(scores);const topTeam=ranked[0];const topPts=teamPts(topTeam.id,scores);
  ...
  function getMVP(){
    return PLAYERS.filter(p=>p.cost>0).map(p=>{const pts=playerPts(p.name,scores);const owner=ownerById(p.owner);const ratio=pts>0?(pts/(p.cost/100)):0;return{p,pts,ratio,owner};}).sort((a,b)=>b.ratio-a.ratio).slice(0,15);
  }
  function getTeamCumulative(ownerId:number):number[]{
    ...
    for(let i=0;i<maxLen;i++){running+=squad.reduce((s,p)=>{const mp=scores[p.name]?.matchPts||[];const mr=mp[i]||0;return s+(isMarquee(p.name)?Math.round(mr*MARQUEE_MULTIPLIER):mr);},0);cum.push(running);}
    return cum;
  }
  ...
  useEffect(()=>{(window as any)['__openPlayerModal']=(name:string)=>{setModalContent(null);setTimeout(()=>openPlayerModal(name),180);};},[scores]);
  ...
  const progressSVG=useCallback(()=>{
    ...
    return `<svg viewBox="0 0 ${W} ${H}" style="width:100%;min-width:300px;height:auto;display:block">${gridLines}${xLabels}${lines}<line x1="${PAD.left}" y1="${PAD.top}" x2="${PAD.left}" y2="${PAD.top+cH}" stroke="#2A3550" stroke-width="1"/><line x1="${PAD.left}" y1="${PAD.top+cH}" x2="${PAD.left+cW}" y2="${PAD.top+cH}" stroke="#2A3550" stroke-width="1"/></svg>`;
  },[scores]);

  const tickerItems=['IPL 2026 · Season Underway','⚡ Marquee system LIVE — 1.5× points for star picks',...OWNERS.flatMap(o=>o.marquee.map(name=>{const p=PLAYERS.find(x=>x.name===name);return `${name} (${o.name}) ⚡ MARQUEE for ${p?.ipl||''}`;}))];
  ...
  function renderRawStats(){
    ...
    const td=(content:string|number,style='')=>`<td style="padding:5px 8px;border:1px solid #1e2a3a;font-size:11px;font-family:'JetBrains Mono',monospace;white-space:nowrap;${style}">${content}</td>`;
    const th=(content:string,style='')=>`<th style="padding:5px 8px;border:1px solid #1e2a3a;font-size:10px;font-weight:700;letter-spacing:1px;background:#0a0f18;white-space:nowrap;text-align:center;${style}">${content}</th>`;
    const matchTitle=(i:number)=>{const m=matchMetas[i];if(!m) return `Match ${i+1}`;return `${m.name}${m.date?' · '+m.date.slice(0,10):''}`;};
    let tableRows='';let teamRunsTotal=0,teamWkTotal=0,teamCatchTotal=0,teamStmpTotal=0;
    const teamMatchPts=Array(numMatches).fill(0);
    for(const p of squad){
      const s=scores[p.name]||{runs:0,wickets:0,catches:0,stumpings:0,matchPts:[]};
      const mq=isMarquee(p.name);const base=rawPts(p.name,scores);const final=playerPts(p.name,scores);
      teamRunsTotal+=s.runs;teamWkTotal+=s.wickets;teamCatchTotal+=s.catches;teamStmpTotal+=s.stumpings;
      const matchCells=matchCols.map(i=>{const mp=s.matchPts[i]??0;const color=mp>0?'#FFD700':'#3a4a60';teamMatchPts[i]=(teamMatchPts[i]||0)+(mq?Math.round(mp*MARQUEE_MULTIPLIER):mp);return td(mp>0?`+${mp}`:'—',`color:${color};text-align:center;`);}).join('');
      tableRows+=`<tr style="${mq?'background:rgba(57,255,20,0.04);':''}">
        ${td(`<span style="color:${owner.color};font-weight:700;">${p.name}${mq?' ⚡':''}</span>`,'min-width:140px;')}
        ${td(p.role,'color:#7A8BAA;text-align:center;')}${td(p.ipl||'—','color:#7A8BAA;text-align:center;')}
        ${td(s.runs,'color:#00D4FF;text-align:right;')}${td(s.wickets,'color:#E8003D;text-align:right;')}
        ${td(s.catches+s.stumpings,'color:#00E676;text-align:right;')}
        ${td(base,'color:#aaa;text-align:right;')}${td(final,`color:#FFD700;font-weight:700;text-align:right;${mq?'text-decoration:underline dotted;':''}`)}
        ${matchCells}</tr>`;
    }
    ...
    const totalsRow=`<tr style="background:#0a0f18;font-weight:700;">${td('TEAM TOTAL','color:#FFD700;font-weight:700;font-size:12px;')}${td('')}${td('')}${td(teamRunsTotal,'color:#00D4FF;text-align:right;font-weight:700;')}${td(teamWkTotal,'color:#E8003D;text-align:right;font-weight:700;')}${td(teamCatchTotal+teamStmpTotal,'color:#00E676;text-align:right;font-weight:700;')}${td(teamBase,'color:#aaa;text-align:right;font-weight:700;')}${td(teamFinal,`color:#FFD700;text-align:right;font-weight:700;font-size:13px;`)}${matchTotalCells}</tr>`;
    const matchHeaders=matchCols.map(i=>th(`<span title="${matchTitle(i)}" style="cursor:help">M${i+1}</span>`,'min-width:44px;')).join('');
    return `<div style="overflow-x:auto;-webkit-overflow-scrolling:touch;"><table style="border-collapse:collapse;width:100%;font-family:'JetBrains Mono',monospace;font-size:11px;"><thead><tr>${th('Player','text-align:left;min-width:140px;')}${th('Role')}${th('IPL')}${th('Runs','color:#00D4FF;')}${th('Wkts','color:#E8003D;')}${th('C+St','color:#00E676;')}${th('Base Pts','color:#aaa;')}${th('Final Pts','color:#FFD700;')}${matchHeaders}</tr></thead><tbody>${tableRows}${totalsRow}</tbody></table></div><div style="margin-top:12px;font-size:10px;color:#7A8BAA;font-family:'JetBrains Mono',monospace;display:flex;gap:16px;flex-wrap:wrap;"><span>Run = 1pt</span><span>Wicket = 25pts</span><span>Catch/Stumping = 5pts</span><span style="color:#39FF14;">⚡ Marquee = ×1.5 final pts</span><span style="color:#aaa;">Match cols = raw base pts</span></div>`;
  }
    ...
    <div style={{background:'#080B10',color:'#E8EDF5',fontFamily:"'Rajdhani',sans-serif",minHeight:'100vh',overflowX:'hidden'}}>
      <style>{`
        ...
        *{margin:0;padding:0;box-sizing:border-box;-webkit-tap-highlight-color:transparent;}
        ...
        .desktop-nav{display:flex;gap:2px;flex-wrap:nowrap;}
        .mobile-nav{display:none;position:absolute;top:60px;left:0;right:0;background:#0A0F18;border-bottom:2px solid #2A3550;z-index:99;padding:6px 8px 10px;animation:navSlideDown 0.2s ease;}
        ...
        .p-row.is-marquee{background:rgba(57,255,20,0.04);}
        ...
        .mp-track-item{display:flex;align-items:center;gap:8px;padding:8px 12px;cursor:pointer;transition:background 0.12s;font-size:11px;}
        ...
        @media(max-width:768px){
          ...
          .lb-row{grid-template-columns:36px 1fr 90px;height:52px;padding:0 .9rem;}
          .lb-col-hide{display:none!important;}
          .teams-grid{grid-template-columns:1fr!important;}
          .pt-row{grid-template-columns:1fr 48px 62px;}
          ...
          .auction-sidebar{display:none!important;}
          ...
          .section-hdr{flex-wrap:wrap;gap:6px!important;}
          .raw-team-btn{font-size:11px;padding:5px 9px;}
      ...
      <header style={{background:'linear-gradient(135deg,#0A0F18,#111827)',borderBottom:'2px solid #FFD700',padding:'0 1rem',position:'sticky',top:0,zIndex:100,boxShadow:'0 4px 40px rgba(255,215,0,0.15)'}}>
        <div style={{maxWidth:1400,margin:'0 auto',display:'flex',alignItems:'center',justifyContent:'space-between',height:60,gap:'1rem',position:'relative'}}>
          <div style={{display:'flex',alignItems:'center',gap:10,flexShrink:0}}>
            <div style={{width:34,height:34,background:'linear-gradient(135deg,#FFD700,#FF6B00)',borderRadius:8,display:'flex',alignItems:'center',justifyContent:'center',fontSize:17,boxShadow:'0 0 20px rgba(255,215,0,0.4)',flexShrink:0}}>🏏</div>
            <div>
              ...
              <div className="header-logo-sub" style={{fontSize:9,color:'#7A8BAA',letterSpacing:3,textTransform:'uppercase',marginTop:-3}}>IPL Fantasy 2026 · 8 Teams</div>
            </div>
          </div>
      ...
      <main style={{maxWidth:1400,margin:'0 auto',padding:'1.5rem',paddingBottom:140}}>
        ...
        {activeView==='leaderboard'&&(
          <div>
            <div className="match-banner" style={{background:'linear-gradient(135deg,#161D2A,#1A2235)',border:'1px solid #2A3550',borderTop:'3px solid #FF6B00',borderRadius:12,padding:'1.25rem 1.5rem',marginBottom:'1.5rem',display:'flex',alignItems:'center',justifyContent:'space-between',boxShadow:'0 8px 32px rgba(0,0,0,0.4)'}}>
              <div style={{display:'flex',alignItems:'center',gap:14,fontFamily:"'Bebas Neue',cursive"}}>
                <div><div style={{fontSize:26,letterSpacing:2,color:'#FF4D4D'}}>RCB</div><div style={{fontSize:12,color:'#7A8BAA',fontFamily:"'Rajdhani',sans-serif",fontWeight:500,marginTop:2}}>203/4 (15.4 ov)</div></div>
                <div style={{fontSize:13,color:'#FFD700',padding:'3px 8px',border:'1px solid #B8960A',borderRadius:4,letterSpacing:2}}>VS</div>
                <div><div style={{fontSize:26,letterSpacing:2,color:'#FF8C00'}}>SRH</div><div style={{fontSize:12,color:'#7A8BAA',fontFamily:"'Rajdhani',sans-serif",fontWeight:500,marginTop:2}}>201/9 (20 ov)</div></div>
              </div>
              <div style={{textAlign:'right'}}>
                <div style={{fontSize:10,letterSpacing:2,color:'#00E676',fontWeight:700,marginBottom:3}}>● MATCH 1 · RESULT</div>
                <div style={{fontSize:11,color:'#7A8BAA'}}>M. Chinnaswamy · 28 Mar 2026</div>
                <div style={{fontSize:12,color:'#FFD700',marginTop:2}}>RCB won by 6 wkts</div>
              </div>
            </div>
            <div className="stats-strip" style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'1rem',marginBottom:'1.5rem'}}>
              {[
                ...
                <div key={i} style={{background:'#161D2A',border:'1px solid #2A3550',borderRadius:10,padding:'1rem 1.1rem',position:'relative',overflow:'hidden'}}>
                  ...
                  <div style={{fontFamily:"'Bebas Neue',cursive",fontSize:i===0?30:15,lineHeight:1,paddingTop:i>0?'7px':0,color:i===3?'#00E676':undefined}}>{box.val}</div>
            ...
            <div className="section-hdr" style={{display:'flex',alignItems:'center',gap:10,marginBottom:'1rem'}}>
              ...
              <div style={{fontSize:9,fontWeight:700,letterSpacing:2,color:'#FFD700',background:'rgba(255,215,0,0.1)',border:'1px solid rgba(255,215,0,0.2)',padding:'3px 8px',borderRadius:3,flexShrink:0}}>{`8 TEAMS · ${matchesPlayed} MATCH${matchesPlayed!==1?'ES':''}`}</div>
            </div>
            ...
            <div className="section-hdr" style={{display:'flex',alignItems:'center',gap:10,margin:'1.5rem 0 1rem'}}>
        ...
        {activeView==='teams'&&(
          <div>
            ...
            <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:'1rem',fontSize:11,color:'#7A8BAA',flexWrap:'wrap'}}>
              <span className="marquee-badge">⚡ MARQUEE</span><span>= 1.5× points multiplier · 3 per team · tap any player</span>
            </div>
          ...
          <div>
            ...
            {matchesPlayed===0?(
              ...
              <div style={{background:'#161D2A',border:'1px solid #2A3550',borderRadius:10,padding:'1rem',overflowX:'auto'}}>
                {matchMetas.length>0&&(
                  <div style={{marginBottom:'12px',display:'flex',flexWrap:'wrap',gap:'6px 14px'}}>
                    {matchMetas.map((m,i)=>(
                  ...
                  <table style={{width:'100%',borderCollapse:'collapse',fontSize:12,fontFamily:"'JetBrains Mono',monospace"}}>
                    <thead><tr style={{background:'#0a0f18'}}>
                      {['Team','Runs','Wickets','Catches+St','Base Pts','Final Pts'].map(h=>(
                        <th key={h} style={{padding:'8px 12px',border:'1px solid #1e2a3a',fontSize:10,fontWeight:700,letterSpacing:1,textAlign:h==='Team'?'left':'right',color:'#7A8BAA',whiteSpace:'nowrap'}}>{h}</th>
                      ))}
                    </tr></thead>
                    <tbody>
                      {sortedTeams(scores).map((owner,i)=>{
                        ...
                        return(<tr key={owner.id} style={{cursor:'pointer',background:rawTeam===owner.id?`${owner.color}10`:undefined}} onClick={()=>setRawTeam(owner.id)}>
      ...
      <div id="music-player" style={{position:'fixed',bottom:'1.25rem',left:'1.25rem',zIndex:400,background:'linear-gradient(135deg,#0D1119,#161D2A)',border:'1px solid #2A3550',borderRadius:14,boxShadow:'0 8px 40px rgba(0,0,0,0.7),0 0 0 1px rgba(255,215,0,0.08)',width:musicCollapsed?175:275,overflow:'hidden',transition:'all 0.3s cubic-bezier(0.34,1.2,0.64,1)',userSelect:'none'}}>
        <div style={{display:'flex',alignItems:'center',gap:9,padding:'9px 11px 7px',borderBottom:'1px solid rgba(42,53,80,0.5)'}}>
          <div style={{width:32,height:32,background:'linear-gradient(135deg,#FFD700,#FF6B00)',borderRadius:'50%',display:'flex',alignItems:'center',justifyContent:'center',fontSize:15,flexShrink:0,animation:isPlaying?'spin-disc 3s linear infinite':undefined,boxShadow:'0 0 12px rgba(255,215,0,0.3)'}}>🎵</div>
          <div style={{flex:1,minWidth:0}}>
            <div style={{fontSize:10,fontWeight:700,letterSpacing:0.5,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis',color:'#E8EDF5'}}>{TRACKS[currentTrack].title}</div>
        ...
        <div style={{display:'flex',alignItems:'center',gap:5,padding:'7px 10px'}}>
          ...
          <button onClick={togglePlay} style={{background:'linear-gradient(135deg,#FFD700,#FF6B00)',color:'#000',width:36,height:36,borderRadius:'50%',display:'flex',alignItems:'center',justifyContent:'center',fontSize:15,boxShadow:'0 0 12px rgba(255,215,0,0.3)',border:'none',cursor:'pointer',flexShrink:0}}>{isPlaying?'⏸':'▶'}</button>
      ...
      {showPinModal&&(
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.85)',backdropFilter:'blur(6px)',zIndex:300,display:'flex',alignItems:'center',justifyContent:'center',padding:'1rem'}}>
          <div style={{background:'#0D1119',border:'1px solid #2A3550',borderRadius:16,padding:'2rem',width:'100%',maxWidth:300,textAlign:'center',boxShadow:'0 24px 80px rgba(0,0,0,0.8)'}}>
      ...
      {/* MODAL */}
      {modalContent&&(
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.82)',backdropFilter:'blur(6px)',zIndex:200,display:'flex',alignItems:'flex-end',justifyContent:'center'}} onClick={e=>{if(e.target===e.currentTarget)setModalContent(null);}}>
          <div className="modal-sheet" style={{background:'#0D1119',border:'1px solid #2A3550',borderRadius:'16px 16px 0 0',width:'100%',maxWidth:560,maxHeight:'88vh',overflowY:'auto',boxShadow:'0 -8px 60px rgba(0,0,0,0.8)',animation:'modalSlideUp 0.28s cubic-bezier(0.34,1.4,0.64,1)'}}>
            ...
            <div style={{padding:'1rem 1.5rem .9rem',borderBottom:'1px solid #2A3550',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
              <div style={{fontFamily:"'Bebas Neue',cursive",fontSize:22,letterSpacing:2,flex:1,minWidth:0,overflow:'hidden'}} dangerouslySetInnerHTML={{__html:modalContent.title}}></div>
              <button onClick={()=>setModalContent(null)} style={{background:'none',border:'1px solid #2A3550',color:'#7A8BAA',width:32,height:32,borderRadius:6,fontSize:14,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,marginLeft:8}}>✕</button>
            </div>
            <div style={{padding:'1.25rem 1.5rem 2rem'}} dangerouslySetInnerHTML={{__html:modalContent.body}}></div>
          </div>
        </div>
      )}
